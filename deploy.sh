...
if [ ! -d ".git" ]; then
    ...
    sudo git init
    sudo git add .
    sudo git commit -m "Initial commit - Preparado para deploy"
    sudo echo "✅ Git inicializado!"
else
...
echo ""
if [[ $REPLY =~ ^[Ss]$ ]]; then
    sudo git commit -m "Deploy: Adicionado gunicorn, health check e configs cloud"
