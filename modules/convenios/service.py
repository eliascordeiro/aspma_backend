...
class ConveniosService:
    ...
    def autenticar(self, usuario: str, senha: str):
        ...
        senha_valida = False
        senha_era_plain_text = False  # Flag para migração automática
        ...
        if senha_mysql:
            ...
            if senha_mysql.startswith('$2') and len(senha_mysql) >= 59:
                ...
                if senha_valida:
                    senha_era_plain_text = True  # Marca para migração

        if not senha_valida:
            # Senha incorreta - bloqueio removido
            raise AuthenticationError('Credenciais inválidas')
        
        # Migração automática: se login foi bem-sucedido com senha plain text, converte para bcrypt
        if senha_era_plain_text and senha:
            try:
                ...
                logging.getLogger(__name__).info(
                    f"Senha do usuário {usuario} (código {codigo}) migrada para bcrypt automaticamente"
                )
            except Exception as e:
                # Não bloqueia login se migração falhar
                import logging
                logging.getLogger(__name__).warning(
                    f"Falha ao migrar senha para bcrypt: {usuario} - {str(e)}"
                )
            ...
            if usuario_token and isinstance(senha, str) and senha:
                ...
                if row_with_pwd:
                    ...
                    if codigo_mysql.upper() == (codigo_convenio or '').strip().upper():
                        ...
                        senha_valida = False
                        senha_era_plain_text = False  # Flag para migração
                        if senha_mysql:
                        ...
                        if senha_valida:
                            ...
                            # Migração automática: se era plain text, converte no MySQL também
                            if senha_era_plain_text:
                                try:
                                    senha_hash_bcrypt = hashpw(senha.encode('utf8'), gensalt())
                                    self.repo.atualizar_senha_mysql(codigo_mysql, senha_hash_bcrypt)
                                    import logging
                                    logging.getLogger(__name__).info(
                                        f"Senha do código {codigo_mysql} migrada para bcrypt (fallback auth)"
                                    )
                                except Exception as e:
                                    import logging
                                    logging.getLogger(__name__).warning(
                                        f"Falha ao migrar senha (fallback): {codigo_mysql} - {str(e)}"
                                    )
